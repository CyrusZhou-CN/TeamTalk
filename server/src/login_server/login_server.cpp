...
int main(int argc, char *argv[])
{
	...
	for (uint32_t i = 0; i < msg_server_listen_ip_list.GetItemCnt(); i++)
	{
		...
		if (ret == NETLIB_ERROR)
		{
			log("!!!error msg_server ip: %s, port: %d", msg_server_listen_ip_list.GetItem(i), msg_server_port);
			return ret;
		}
	}
	...
	for (uint32_t i = 0; i < http_listen_ip_list.GetItemCnt(); i++)
	{
		...
		if (ret == NETLIB_ERROR)
		{
			log("!!!error http_msg_server ip: %s, port: %d", http_listen_ip_list.GetItem(i), http_port);
			return ret;
		}
	}
