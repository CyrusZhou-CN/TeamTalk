...
int main(int argc, char *argv[])
{
	...
	srand(time(NULL));
	CacheManager *pCacheManager = nullptr;
	int retryCount = 0;
	while (true)
	{
		pCacheManager = CacheManager::getInstance();
		if (pCacheManager)
		{
			// 初始化成功
			break;
		 ...
		 retryCount++;
    	log("Retrying to initialize CacheManager (Attempt: %d)", retryCount);
		sleep(1); // 等待1秒
	}
	if (!pCacheManager)
	{
	...
	while (true)
	{
		...
		retryCount++;
		log("Retrying to initialize DBManager (Attempt: %d)", retryCount);
		sleep(1); // 等待1秒
	}
	if (!pDBManager)
	{
