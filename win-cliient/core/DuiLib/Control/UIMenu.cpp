...
namespace DuiLib {
	...
	{
		//CString strOutPut;
		//strOutPut.Format(_T("CMenuUI::DoEvent-UIEVENT_BUTTONDOWN %d \n\r"), event.Type);
		//OutputDebugString(strOutPut);
		if (event.Type == UIEVENT_BUTTONDOWN)
		{
			//OutputDebugString(_T("CMenuUI::DoEvent-UIEVENT_BUTTONDOWN\\n"));
		}
	...
	BOOL CMenuWnd::Receive(ContextMenuParam param)
	{
		...
		case 1:
		{
			...
			::SendMessage(m_hParent, WM_MENU_NOTIFY, WPARAM(param.name.GetData()), LPARAM(m_sUserData.GetData()));			
			Close();
			break;
		}
		case 2:
	...
	const CDuiString& CMenuWnd::GetUserData()
	{
		return m_sUserData;
	}

	void CMenuWnd::SetUserData(LPCTSTR pstrText)
	{
		m_sUserData = pstrText;
	}
	void CMenuWnd::AdjustPostion(const POINT& point)
	{
		OutputDebugString(_T("AdjustPostion"));
		CDuiRect rcWnd;
